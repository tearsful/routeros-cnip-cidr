...
EOF
cat $WORK_DIR/tmp/all_cn.txt | awk '{ printf(":do {add cmment=cn_ip_cidr address=%s list=cn_ip_cidr} on-error={}\n",$0) }' >> $WORK_DIR/dist/cn_ip_cidr.rsc && \
cat >> $WORK_DIR/dist/cn_ip_cidr.rsc << EOF
:if ([:len [/system package find where name="ipv6" and disabled=no]] > 0) do={
/log info "Import cn ipv6 cidr list..."
/ipv6 firewall address-list remove [/ipv6 firewall address-list find comment=cn_ip6_cidr]
/ipv6 firewall address-list
EOF
cat $WORK_DIR/tmp/all_cn_ipv6.txt | awk '{ printf(":do {add comment=cn_ip6_cidr address=%s list=cn_ip6_cidr} on-error={}\n",$0) }' >> $WORK_DIR/dist/cn_ip6_cidr.rsc && \
echo "}" >> $WORK_DIR/dist/cn_ip6_cidr.rsc
